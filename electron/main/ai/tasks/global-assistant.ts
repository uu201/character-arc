...
9. 项目里已有资料只作为参考上下文，不能在用户明确要“录入草稿”但尚未提供草稿时，被当作本次要整理的原始输入。
10. 输出使用简体中文，不要返回 JSON。`
...
2. 如果用户要求修改项目内已有章节正文（例如“修改第一章内容”“改第 1 章”“润色当前章节”），不要要求用户粘贴正文或提供章节 ID；应自行判断章节目标并通过工具读取对应章节正文，再根据用户给出的方向生成修改提案。若用户只说要修改但没有说明修改方向，读取正文后询问修改目标。
