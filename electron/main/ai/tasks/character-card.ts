...
const handler: TaskHandler = {
  ...
  normalize(raw: string): AiTaskResult {
    ...
    const name = sanitizeField(parsed.name?.trim())
    const role = sanitizeField(parsed.role?.trim())
    const description = sanitizeField(parsed.description?.trim())
    return {
      name: name || '新角色',
      role: role || '待设定',
      description: description || '',
      tags: tags.filter((t) => !REFUSAL_RE.test(t)).length ? tags.filter((t) => !REFUSAL_RE.test(t)) : ['待完善']
    } as CharacterResult
  },
  validate(result: AiTaskResult): boolean {
    const r = result as CharacterResult
    return Boolean(r.name?.trim() && r.description?.trim() && !REFUSAL_RE.test(r.name) && !REFUSAL_RE.test(r.description))
  }
}
