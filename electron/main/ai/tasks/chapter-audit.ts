...
const handler: TaskHandler = {
  ...
  buildPrompt(input: PromptBuildInput) {
    ...
    return {
      system: `${capabilityPreamble.system}\n\n你是小说章节审计师。审计 Writer 刚生成的初稿是否在正文里真的兑现了章节执行包和写作备忘里的硬契约。只返回 JSON，不要 markdown。

审计维度（按这个顺序逐项检查）：
1. mustInclude / payoffs 兑现：执行包 mustInclude 与备忘 payoffs 列表里的每一条，是否在正文里有可定位的具体动作 / 物件 / 对话兑现段（不能只是侧面提一嘴或内心提及）
2. endingHook / endingChanges 落地：执行包 endingHook 与备忘 endingChanges 的每一条改变，是否真的在章末发生（信息变化 / 关系变化 / 物理变化 / 权力变化）
3. mustNotInclude / holds / doNotDo 守住：禁止项、底牌和红线是否被压住没掀，是否被触碰
4. continuityNotes：是否承接相邻章节、大纲拆章和人物状态，没有明显断裂
5. qualityGates：执行包里的质量门禁是否被满足，尤其是开场、兑现、章末钩子与字数范围
6. 开头钩子：前 100 字是否有具体动作 / 对话 / 反差 / 信息冲击 / 未完成动作
7. 章末钩子：是否有未完成动作或新信息把读者拉向下章（不是总结式收尾、鸡汤升华、廉价预告）
8. 字数：实际字数是否在目标 ±20% 内
9. 硬规则：是否出现破折号（——）、高疲劳词（冷笑/瞳孔骤缩/轰然炸裂/倒吸一口凉气/蝼蚁等）、章节内分隔符（---、#）

issue 格式：
- severity: "critical"（payoff 漏兑现 / endingChange 未发生 / 触碰红线）/"warning"（钩子弱 / 字数偏离）/"hint"（高疲劳词 / 句式单一）
- category: "must-include" / "payoff" / "ending-change" / "must-not-include" / "hold" / "do-not-do" / "continuity" / "quality-gate" / "opening-hook" / "ending-hook" / "word-count" / "hard-rule"
...
pass 判定：所有 critical issue 数 == 0 且 warning issue 数 <= 2 即 pass。`,
      user: `${capabilityPreamble.user}\n\n请审计以下章节初稿。\n\n${executionPacketBlock ? `${executionPacketBlock}\n\n` : ''}${memoBlock}\n\n目标字数：${targetWordCount}\n\n## 章节正文\n\n${draftText}\n\n返回审计 JSON。`
    }
  },
