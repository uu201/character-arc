...
const handler: TaskHandler = {
  ...
  validate(result: AiTaskResult): boolean {
    ...
    const organizationComplete = Boolean(r.name && r.type && r.description && r.motto)
    const relationshipComplete = Boolean(r.type && r.description && typeof r.intensity === 'number')
    const membershipComplete = Boolean(r.role && r.notes)
    return organizationComplete || relationshipComplete || membershipComplete
  },
  describeValidationErrors(): string[] {
    return ['返回内容不完整：组织需包含 name/type/description/motto；关系需包含 type/description/intensity；归属需包含 role/notes。']
  }
}
