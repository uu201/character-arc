...
export function makeStageChapterCreateTool(deps: StageChapterCreateToolDeps): Tool {
  return {
    definition: {
      ...
      description:
        '暂存新建章节，不直接写库。用于"新增一章""按大纲铺开新章节""生成新章节初稿"等场景。title 必填；可选 summary（章节摘要）、volume_id（所属分卷，缺省用当前分卷或第一个分卷）、word_target、content（章节初稿正文纯文本，工具会转成段落）。用户在暂存区确认后才真正创建。',
      inputSchema: {
        type: 'object',
        properties: {
    ...
    handler: async (input) => {
      ...
      const wordTarget = readString(input, 'word_target')
      ...
      return {
        content: [
          `已暂存新建章节（change_id=${change.id}）：${title}${volume ? `（分卷：${volume.title}）` : ''}。`,
